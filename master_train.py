...
class VAEConfig:
    ...
    def __init__(self, args):
        self.latent_dims = args.vae_latent_dims
...
class RNNConfig:
    ...
    def __init__(self, args):
        self.input_size = args.vae_latent_dims + args.n_actions
        self.hidden_size = args.rnn_hidden_size
        self.output_size = args.vae_latent_dims
        self.n_gaussians = args.rnn_n_gaussians
        self.n_layers = args.rnn_layers
        self.temp = args.rnn_temp
...
class MDNRNN(nn.Module):
    ...
    def initial_state(self, batch_size):
        return (
            torch.zeros(self.layers, batch_size, self.output_size),
...
class ControllerConfig:
    ...
    def __init__(self, args):
        self.input_size = args.rnn_hidden_size + args.vae_latent_size
...
class Controller:
    def __init__(self, data, config):
        ...
        self.w = data[: self.input_size * self.act_size].reshape(
...
def rollout(worker_id, trials, args):
    ...
    for trial in range(trials):
        ...
        print(
            f"rollout worker {worker_id} [{trial+1}/{trials}] ({total_frames} frames)"
        )
...
def generate_rollouts(args):
    ...
    with Pool(workers) as pool:
        total_frames = sum(
            pool.starmap(
                rollout, [(i, trials_per_worker, args) for i in range(workers)]
            )
        )
...
def train_vae(args):
    ...
    # Init wandb
    vae_args = {
        k.replace("vae_", ""): v for k, v in vars(args).items() if k.startswith("vae_")
    }
    wandb.init(
        project="world-models", config=vae_args, name=f"vae_{wandb.util.generate_id()}"
    )
    ...
    with tqdm(range(args.vae_epochs + 1)) as t:
        for epoch in t:
            for x in dataloader:
                ...
                x = x.permute(
                    0, 3, 1, 2
                ).contiguous()  # Get channels to be first dimension
                x_hat = model(x)
...
def generate_latents(args):
    ...
    for file in glob.glob(f"{args.rollout_dir}/*.npz"):
        ...
        np.savez_compressed(
            latent_file, mu=mu.cpu().numpy(), logvar=logvar.cpu().numpy(), act=act
        )
...
def train_rnn(args):
    ...
    rnn_args = {
        k.replace("rnn_", ""): v for k, v in vars(args).items() if k.startswith("rnn_")
    }
    wandb.init(
        project="world-models", config=rnn_args, name=f"rnn_{wandb.util.generate_id()}"
    )
...
def rollout(solution, args):
    ...
    vae = VAE(VAEConfig(args)).to(device)
    rnn = MDNRNN(RNNConfig(args)).to(device)
    c_config = ControllerConfig(args)
    controller = Controller(solution, c_config)

    if args.compile:
        vae = torch.compile(vae, "max-autotune")
        rnn = torch.compile(rnn, "max-autotune")
    ...
    for _ in range(args.controller_evals_per_agent):
        ...
        while not done:
            ...
            _, _, _, h = rnn(
                torch.cat([z.flatten(), a], dim=-1).unsqueeze(0).unsqueeze(0), h
            )
            # h = (h[0].to(device), h[1].to(device))
...
def train_controller(args):
    ...
    es = cma.CMAEvolutionStrategy(
        (c_config.input_size * c_config.act_size + c_config.act_size) * [0], 0.5
    )
    while not es.stop():
        solutions = es.ask()
        with Pool(workers) as pool:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--phases",
        nargs="+",
        choices=["rollout", "vae", "latent", "rnn", "controller"],
    ...
    parser.add_argument(
        "--rollout-dir", type=str, default="rollouts", help="directory to save rollouts"
    )
    parser.add_argument(
        "--n-workers",
        type=int,
        default=64,
        help="number of workers to use for rollouts and loading",
    )
    parser.add_argument(
        "--n-rollouts", type=int, default=5_000, help="number of rollouts to extract"
    )
    parser.add_argument(
        "--max-frames",
        type=int,
        default=1000,
        help="maximum number of frames per rollout",
    )
    # phase 2. train VAE
    ...
    parser.add_argument(
        "--vae-epochs", type=int, default=5, help="number of epochs to train the VAE"
    )
    parser.add_argument(
        "--vae-batch-size", type=int, default=128, help="batch size for training"
    )
    parser.add_argument(
        "--vae-learning-rate",
        type=float,
    ...
    parser.add_argument(
        ...
        type=str,
        default="series",
        help="directory to save latent space dataset",
    )
    # phase 4. train RNN
    parser.add_argument(
        "--dir-rnn",
        type=str,
        default="rnn_checkpoints",
        help="directory to save RNN checkpoints",
    )
    parser.add_argument(
        "--rnn-hidden-size",
        type=int,
    ...
    parser.add_argument(
        "--rnn-n-gaussians", type=int, default=5, help="number of gaussians for the MDN"
    )
    parser.add_argument(
        "--rnn-layers", type=int, default=1, help="number of layers in the RNN"
    )
    parser.add_argument(
        "--rnn-temp", type=float, default=1.0, help="temperature for the MDN"
    )

    parser.add_argument(
        "--rnn-epochs", type=int, default=100, help="number of epochs to train the VAE"
    )
    parser.add_argument(
        "--rnn-batch-size", type=int, default=32, help="batch size for training"
    )
    parser.add_argument(
        "--rnn-learning-rate",
        type=float,
    ...
    parser.add_argument(
        "--dir-controller",
        type=str,
        default="controller_checkpoints",
        help="directory to save controller checkpoints",
    )
    parser.add_argument(
        "--controller-pop-size",
        type=int,
        default=64,
        help="population size for controller training",
    )
    parser.add_argument(
        "--controller-evals-per-agent",
        type=int,
        default=8,
        help="number of evaluations per agent",
    )
